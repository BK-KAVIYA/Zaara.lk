<?php
  ...
  /*  if(isset($_SESSION['digimart_current_user_id'])){
    ...
    if(isset($_GET['setDefault'])){
        ...
        $sql = "UPDATE `customer_mail_info` SET `is_default`= 0 WHERE `customer_id` = '{$_SESSION['uid']}'";
        mysqli_query($conn, $sql);
        
        $sql = "UPDATE `customer_mail_info` SET `is_default`= 1 WHERE `customer_id` = '{$_SESSION['uid']}' AND `id` = $id";
        mysqli_query($conn, $sql);
    ...
    if(isset($_POST['btnSubmit'])){
        ...
        $sql = "INSERT INTO `customer_mail_info`(`customer_id`, `name`, `street_1`, `street_2`, `city`, `zip_code`, `mobile_no`) VALUES ('{$_SESSION['uid']}', '{$name}', '{$street1}', '{$street2}', '{$city}', $zip, '{$mobile}')";
...
<head>
    <!-- title -->
	<title>My Mail Address | Zaara.lk</title>
            ...
            <div class="row mw-100 p-2" id="product-container">
                ...
                <div class="">
                    <div class="col-12 d-flex pl-5" id="mailCard">
                        <div class="card border-danger m-2 text-white bg-dark">
